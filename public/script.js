...
function updateTimerDisplay() {
  if (!timerControls || !timerControls.display) {
    return;
  }
  const remaining = getTimerRemaining();
  timerControls.display.textContent = formatTimerDisplay(remaining);
  if (!timerState.isRunning && remaining === 0) {
    timerControls.display.classList.add('timer-finished');
  } else {
    timerControls.display.classList.remove('timer-finished');
  }
}
...
function updateTimerInputs() {
  ...
  const minutesInput = timerControls.minuteInput;
  const secondsInput = timerControls.secondInput;
  if (minutesInput && document.activeElement !== minutesInput) {
    const minutes = Math.floor(timerState.duration / 60000);
    minutesInput.value = String(minutes);
  }
  if (secondsInput && document.activeElement !== secondsInput) {
    const seconds = Math.floor((timerState.duration % 60000) / 1000);
    secondsInput.value = String(seconds).padStart(2, '0');
  }
}

function updateTimerStatus(message, isError = false) {
  if (!timerControls || !timerControls.statusElement) {
    return;
  }
  timerControls.statusElement.textContent = message;
  if (isError) {
    timerControls.statusElement.classList.add('error');
  } else {
    timerControls.statusElement.classList.remove('error');
  }
}

function isTimerConnected() {
  return Boolean(timerEventSource && timerEventSource.readyState === EventSource.OPEN);
}

function updateTimerControlsState() {
  if (!timerControls) {
    return;
  }
  const isConnected = isTimerConnected();
  const canSendCommands = Boolean(timerEventSource && timerEventSource.readyState !== EventSource.CLOSED);
  const { startStopButton, resetButton, applyButton, minuteInput, secondInput } = timerControls;

  if (startStopButton) {
    if (timerState.isRunning) {
      startStopButton.textContent = '일시정지';
    } else if (getTimerRemaining() === 0) {
      startStopButton.textContent = '다시 시작';
    } else {
      startStopButton.textContent = '시작';
    }
    startStopButton.disabled = !canSendCommands;
  }

  if (resetButton) {
    resetButton.disabled = !canSendCommands;
  }

  const disableInputs = !canSendCommands || timerState.isRunning;
  if (applyButton) {
    applyButton.disabled = disableInputs;
  }
  if (minuteInput) {
    minuteInput.disabled = disableInputs;
  }
  if (secondInput) {
    secondInput.disabled = disableInputs;
  }
}
...
function applyTimerState(state) {
  ...
  timerState.isRunning = Boolean(state.isRunning);
  timerState.endTime = typeof state.endTime === 'number' ? state.endTime : null;
  timerState.updatedAt = typeof state.updatedAt === 'number' ? state.updatedAt : Date.now();
...
function getDurationFromInputs() {
  ...
  const minutes = Number(timerControls.minuteInput?.value ?? 0);
  const seconds = Number(timerControls.secondInput?.value ?? 0);
  ...
  const total = safeMinutes * 60000 + clampedSeconds * 1000;
  return clampTimerDuration(total);
}
...
async function fetchTimerState() {
  try {
    ...
    if (!response.ok) {
      throw new Error('현재 타이머 정보를 불러오지 못했습니다.');
    }
    const data = await response.json();
    applyTimerState(data);
  } catch (error) {
    console.error(error);
    updateTimerStatus(error.message || '현재 타이머 정보를 불러오지 못했습니다.', true);
  }
}

async function sendTimerRequest(endpoint, payload) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || '타이머 요청 처리 중 문제가 발생했습니다.');
    }
    ...
    updateTimerStatus('타이머 요청이 완료되었습니다.');
  } catch (error) {
    console.error(error);
    updateTimerStatus(error.message || '타이머 요청 처리 중 문제가 발생했습니다.', true);
  }
}
...
function initTimerControls() {
  ...
  const openButton = document.getElementById('open-timer-modal');
  const closeButton = document.getElementById('close-timer-modal');
  const minuteInput = document.getElementById('timer-minute-input');
  const secondInput = document.getElementById('timer-second-input');
  const applyButton = document.getElementById('timer-apply');
  const startStopButton = document.getElementById('timer-start-stop');
  ...
  function openModal() {
    ...
    updateTimerControlsState();
    window.setTimeout(() => {
      if (!timerState.isRunning && minuteInput && !minuteInput.disabled) {
        minuteInput.focus();
      } else if (startStopButton && !startStopButton.disabled) {
  ...
  secondInput.addEventListener('change', () => {
    ...
    secondInput.value = String(Math.min(59, safeSeconds)).padStart(2, '0');
  });
  ...
  minuteInput.addEventListener('change', () => {
    ...
    minuteInput.value = String(Math.min(180, safeMinutes));
  });

  applyButton.addEventListener('click', () => {
    const duration = getDurationFromInputs();
    if (!Number.isFinite(duration) || duration <= 0) {
      alert('타이머 시간을 5초 이상으로 설정해주세요.');
      updateTimerInputs();
      return;
    }
    sendTimerRequest('/api/timer/duration', { duration });
  });

  startStopButton.addEventListener('click', () => {
    if (timerState.isRunning) {
      sendTimerRequest('/api/timer/pause');
      return;
    }

    const remaining = getTimerRemaining();
    if (remaining > 0 && remaining < timerState.duration) {
      sendTimerRequest('/api/timer/start');
      return;
    }

    const duration = getDurationFromInputs();
    sendTimerRequest('/api/timer/start', { duration });
  });

  resetButton.addEventListener('click', () => {
    sendTimerRequest('/api/timer/reset');
  });

  timerEventSource = new EventSource('/api/timer/stream');

  timerEventSource.onopen = () => {
    updateTimerStatus('실시간으로 연결되었습니다.');
    updateTimerControlsState();
    fetchTimerState();
  };

  timerEventSource.onmessage = (event) => {
    try {
  ...
  timerEventSource.onerror = () => {
    updateTimerStatus('연결이 불안정합니다. 잠시 후 다시 시도해주세요.', true);
    updateTimerControlsState();
    if (timerEventSource.readyState === EventSource.CLOSED) {
      fetchTimerState();
    }
  };
